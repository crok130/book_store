...
public interface ChatDAO {
	...
	@Select("SELECT c.chatroom_num, c.tradebook_num as book_num, " +
			"CASE WHEN c.seller_member_num = #{member_num} THEN b.member_nickname ELSE s.member_nickname END as opponent_nickname, " +
			"t.tradebook_title, " +
			"m.message_content, " +
			"m.sent_at " +
			"FROM chatrooms c " +
			...
			"LEFT JOIN (SELECT chatroom_num, message_content, sent_at, " +
			"          ROW_NUMBER() OVER (PARTITION BY chatroom_num ORDER BY sent_at DESC) as rn " +
			"          FROM messages) m ON m.chatroom_num = c.chatroom_num AND m.rn = 1 " +
