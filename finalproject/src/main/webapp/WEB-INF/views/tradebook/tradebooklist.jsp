            ...
            <div class="exchange-grid">
              <c:forEach var="item" items="${list}">
                <div class="exchange-card">
                  <c:choose>
                    <c:when test="${not empty item.tradebook_img}">
                      <img src="${path}${item.tradebook_img}" alt="${item.tradebook_title}" class="exchange-book-image"/>
                    </c:when>
                    <c:otherwise>
                      <img src="https://images.unsplash.com/photo-1543002588-bfa74002ed7e?w=400&h=300&fit=crop" alt="placeholder" class="exchange-book-image"/>
                    </c:otherwise>
                  </c:choose>
                  <div class="exchange-info">
                    ...
                    <div class="exchange-details">
                      <span class="exchange-condition">${item.tradebook_condition}</span>
                      <span class="exchange-date">${item.tradebook_time}</span>
                    </div>
                ...
                <nav>
                  <ul class="pagination justify-content-center mb-0">
                    <c:if test="${pm.moveFirst}">
                      <li class="page-item"><a class="page-link" href="${path}/tradebook/list${pm.makeQuery(1)}">처음</a></li>
                    </c:if>
                    <c:if test="${pm.movePrev}">
                      <li class="page-item"><a class="page-link" href="${path}/tradebook/list${pm.makeQuery(pm.prev)}">이전</a></li>
                    </c:if>
                    <c:forEach var="i" begin="${pm.startPage}" end="${pm.endPage}">
                      <li class="page-item ${pm.criteria.page eq i ? 'active' : ''}">
                        <a class="page-link" href="${path}/tradebook/list${pm.makeQuery(i)}">${i}</a>
                      </li>
                    </c:forEach>
                    <c:if test="${pm.moveNext}">
                      <li class="page-item"><a class="page-link" href="${path}/tradebook/list${pm.makeQuery(pm.next)}">다음</a></li>
                    </c:if>
                    <c:if test="${pm.moveLast}">
                      <li class="page-item"><a class="page-link" href="${path}/tradebook/list${pm.makeQuery(pm.last)}">마지막</a></li>
                    </c:if>
                  </ul>
                </nav>
              </div>
